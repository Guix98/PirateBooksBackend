...
private ProductsBl productsBl;
    ...
    @RequestMapping(
            ...
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<ProductsModel>> findAllProducts(@RequestHeader("Authorization") String authorization, @RequestBody String json) throws JSONException { // bearer asdasdasdasd
        ...
        return new ResponseEntity<>( this.productsBl.findAllProducts(), HttpStatus.OK);
    }
    @RequestMapping(
            value = "find",
            method = RequestMethod.POST,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<List<ProductsModel>> findProductsbyID(@RequestHeader("Authorization") String authorization, @RequestBody String json) throws JSONException { // bearer asdasdasdasd
        ...
        int order = Integer.parseInt(jsonObject.getString("product_id"));
        System.out.println("ORDER_NAME: " + order);
        if(!"AUTHN".equals(decodedJWT.getClaim("type").asString()) ) {
            throw new RuntimeException("El token proporcionado no es un token de Autenthication");
        }
        ...
        return new ResponseEntity<>( this.productsBl.findProductsbyID(order), HttpStatus.OK);
    }
